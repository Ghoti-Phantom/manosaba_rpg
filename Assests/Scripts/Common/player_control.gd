...
@export var animation_behaviour_script_init: Node3D
...
func _ready() -> void:
	...
	animation_behaviour_script = animation_behaviour_script_init as EmmaAnimationBehaviour
...
func _process(delta: float) -> void:
	...
	if move_dir.x >= 0:
		velocity.x = move_dir.x * speed - 0.02 # 这个0.01是给动画做的调整，反正几乎不影响速度
	else:
		velocity.x = move_dir.x * speed + 0.02
